...
class domain_fronting(threading.Thread):
    ...
    def log(self, value, status='Bismillahirahmanirrahim', color='[R1]'):
    ...
    def run(self):
        try:
            ...
            self.log('belum konek '.format(self.proxy_host, self.proxy_port))
            ...
            self.log('udah konek ''[CC]'.format(self.proxy_host, self.proxy_port), color='[CC]')
        except OSError:
            self.log('Connection lemot', color='[CC]')
        except TimeoutError:
