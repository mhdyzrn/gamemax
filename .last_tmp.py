...
def log(value, status='Inject Running', color='[CC]'):
...
class domain_fronting(threading.Thread):
    ...
    def log(self, value, status='Ridha Tutorial>>>', color='[R1]'):
    ...
    def run(self):
        try:
            ...
            self.log('Belum Rezeki '.format(self.proxy_host, self.proxy_port))
            ...
            self.log('Rezeki  ''[CC]'.format(self.proxy_host, self.proxy_port), color='[CC]')
        except OSError:
            self.log('Connection Males', color='[CC]')
        except TimeoutError:
            self.log('{} Not Responding'.format(self.proxy_host), color='[R1]')

def main():
    print(colors('\n'.join([
     '[R1]Script Gamexmax Ridha Tutorial','[CC]'
     '[R1] Subscribe Now','[CC]'
     'Semoga Connect Di Tkp Kalian','[CC]'
     'IG :@Radhian_husnil (follow)','[CC]'
